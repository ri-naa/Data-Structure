...
void insert_customer(int arrival_time)
{
	...
	customer.service_time = (int)(max_serv_time * random()) + 1; // 1<= servvice_time <6
...
int remove_customer_a() {
	element customer;
	int a_service_time; 
	                                                                                                              
	if (is_empty(&queue)) return 0;
	customer = dequeue(&queue);
	a_service_time = customer.service_time - 1;
	served_customers++;
	waited_time += clock - customer.arrival_time;
	printf("A server : Customer %d starts service in %d minutes. Wait time was %d minutes.\n", customer.id, clock, clock - customer.arrival_time);
	return a_service_time;
}
int remove_customer_b() {
	element customer;
	int b_service_time;

	if (is_empty(&queue)) return 0;
	customer = dequeue(&queue);
	b_service_time = customer.service_time - 1;
	served_customers++;
	waited_time += clock - customer.arrival_time;
	printf("B server : Customer %d starts service in %d minutes. Wait time was %d minutes.\n", customer.id, clock, clock - customer.arrival_time);
	return b_service_time;
}
...
void main()
{
	...
	while (clock < duration) {
		...
		if (a_service_time > 0 && b_service_time > 0) {
			a_service_time--;
			b_service_time--;
		}	// the customer is receiving service
			
		else if (a_service_time == 0 && b_service_time > 0) {
			a_service_time = remove_customer_a();
			b_service_time--;
		}

		else if (a_service_time > 0 && b_service_time == 0) {
			a_service_time--;
			b_service_time = remove_customer_b();
		}
