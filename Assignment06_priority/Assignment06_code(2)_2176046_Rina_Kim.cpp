...
int a[MAX_BIT] = { 0 };
void huffman_traversal(TreeNode *node, int top)	//root node
{
	if (node->l) {
		a[top] = 0;
		huffman_traversal(node->l, top+1);
		a[top] = 1;
		huffman_traversal(node->r, top+1);
	}
...
void print_codeword()
{
	...
	for (int i = 0; i < m_char_size; i++)
	{
		...
		for (int j = 0; j < m_bit_size[i]; j++) //m_bit_size는 각 문자들 codeword의 비트 개수
			printf("%d", m_LUT[i][j]);
...
void huffman_encoding(char *str, bits_stream *bits_str)
{
	for (int i = 0; i < strlen(str); i++) {
		...
		for (int j = 0; j < m_bit_size[ind]; j++) { 
			bits_str->stream[interval + j] = m_LUT[ind][j]; 
		}
		interval += m_bit_size[ind];
	}
...
void main()
{
	...
	// Generate the huffman codeword from the huffman binary tree
	huffman_traversal(element_root.ptree,0);
